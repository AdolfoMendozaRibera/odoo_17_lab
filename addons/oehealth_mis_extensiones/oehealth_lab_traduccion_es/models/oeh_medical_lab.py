...
class OeHealthLabRequest(models.Model):
    _inherit = 'oeh.medical.lab.request'

    def add_sample(self):
        # Ejecutamos la función original para obtener toda la lógica (wizard, context, etc.)
        res = super(OeHealthLabRequest, self).add_sample()
        # Solo sobreescribimos el nombre que verá el usuario
        if isinstance(res, dict):
            res['name'] = "Agregar Muestra"
        return res