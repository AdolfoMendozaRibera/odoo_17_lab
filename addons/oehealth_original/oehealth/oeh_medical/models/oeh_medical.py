##############################################################################
#    Copyright (C) 2015 Braincrew Apps (<http://www.braincrewapps.com>). All Rights Reserved
...
class OeHealthPatient(models.Model):
    ...
    def action_open_attachments(self):
        ...
        return {
            ...
            'context': "{'search_default_res_name': '%s','default_res_name': '%s'}" % (self.display_name,self.display_name),
            'help': """
                <p class="o_view_nocontent_smiling_face">Add attachments for this patient</p>
                """,
        }
...
class OeHealthAppointment(models.Model):
    ...
    doctor = fields.Many2one('oeh.medical.physician', string='Doctor', help="Current primary care / family doctor",
                             domain=[('is_pharmacist', '=', False)], required=True, readonly=False, default=_get_physician)
    appointment_date = fields.Datetime(string='Appointment Date', required=True, readonly=False, default=datetime.datetime.now())
    appointment_end = fields.Datetime(compute=_get_appointment_end, string='Appointment End Date', readonly=False)
    duration = fields.Float(string='Duration (HH:MM)', readonly=False, default=lambda self: self.env.company.appointment_duration)
    institution = fields.Many2one('oeh.medical.health.center', string='Health Center', help="Medical Center",
                                  readonly=False)
    urgency_level = fields.Selection(URGENCY_LEVEL, string='Urgency Level', readonly=False, default=lambda *a: 'Normal')
    comments = fields.Text(string='Comments', readonly=False)
    patient_status = fields.Selection(PATIENT_STATUS, string='Patient Status', readonly=False, default=lambda *a: 'Inpatient')
