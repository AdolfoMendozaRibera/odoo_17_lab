...
class oeHealthRESTAPIController(http.Controller):
    ...
    def get_patient_list(self, **payload):
        ...
        model = request.env[self._model].sudo().search([('model', '=', model_name)], limit=1)
        payload[
            'fields'] = "['id', 'name', 'dob', 'identification_code', 'image_512', 'age', 'marital_status', 'sex', 'blood_type', 'rh', 'prescription_count']"
                    ...
                    if str(k) == "prescription_line":
                        ...
                        if v and len(v) > 0:
                            for prescription_id in v:
                                prescription = request.env['oeh.medical.prescription.line'].sudo().browse(int(prescription_id))
                                if prescription:
    ...
    def get_labtest_list(self, user_type=None, user_id=None, **payload):
        ...
        model = request.env[self._model].sudo().search([('model', '=', model_name)], limit=1)
        payload['fields'] = "['id', 'name', 'patient', 'pathologist', 'requestor', 'date_requested', 'date_analysis', 'results', 'state', 'lab_test_criteria']"
    ...
    def patient_registration(self, **post):
        ...
        if model:
            try:
                username, password, firstname, lastname = post.get('login'), post.get('password'), post.get('firstname'), post.get('lastname')
                if not all([username, password, firstname,  lastname]):
                    ...
                    error_msg = 'either of the following are missing [login, password, firstname, lastname]'
                    return invalid_response('missing error', error_msg)
                ...
                if partner:
                    patient_values = {}
                    search_patient = request.env[model_name].sudo().search([('firstname','=', firstname),
                                                                            ('lastname','=', lastname)], limit=1)
                    if search_patient:
                        return invalid_response('patient already exists', 'Patient already exists !')

                    patient_values['firstname'] = firstname
                    patient_values['lastname'] = lastname
